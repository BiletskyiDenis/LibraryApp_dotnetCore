...
namespace LibraryServices
{
    public class ImageHandlerService : IImageHandlerService
    {
        ...
        public string GenerateImageFileName(params string[] args)
        {
            ...
            {
                var pieceOfFileName = Regex.Replace(arg, @"[^a-zA-z0-9]+", String.Empty);

                if (pieceOfFileName.Length > 40)
                {
                    pieceOfFileName = pieceOfFileName.Substring(0, 40);
                }

                fileName += pieceOfFileName;
            }

            return fileName + random.Next(1000, 9999).ToString();
        }
